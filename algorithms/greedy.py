...
def combined_heuristic(release_times: List[int], processing_times: List[int], 
                       ...
                       num_runways: int) -> List[List[int]]:
    """
    Greedy algorithm that combines consideration of release times and penalties.
    ...
    # Create a list of flight indices sorted by a combined metric
    # (weighting both the release time and the penalty)
    flight_indices = list(range(num_flights))
    ...
    # Sort by a score that prioritizes early release times and high penalties
    # Lower score = higher priority
    flight_indices.sort(
        key=lambda i: (release_times[i] / max_release) - (penalties[i] / max_penalty)
    )
    ...
    # Assign each flight to the runway that can process it with minimum delay * penalty
    for flight in flight_indices:
        min_cost = float('inf')
        best_runway = -1
        
        # Find the runway that minimizes the cost for this flight
        for runway in range(num_runways):
            if last_flight_on_runway[runway] == -1:
                # No previous flight on this runway
                possible_start_time = max(runway_times[runway], release_times[flight])
            ...
            # Calculate the delay and cost (delay * penalty)
            delay = max(0, possible_start_time - release_times[flight])
